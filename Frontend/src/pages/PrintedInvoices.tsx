...
import { getPrintedInvoices, getInvoiceById, getCompanyById } from '../services/apiservices';
...
const PrintedInvoices: React.FC = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const invoices = await Promise.all(
          (data.printed || data).map((p: any) => getInvoiceById(p.invoice_id, token))
        );
        setPrintInvoices(invoices);

        const companies = await Promise.all(
          invoices.map((inv: any) =>
            inv?.company_id ? getCompanyById(inv.company_id, token).then(res => res.company) : null
          )
        );
  ...
  // Filtros y selección
  const filteredPrinted = printed.filter((row, idx) => {
    const invoice = printInvoices.find((inv: any) => inv && inv.id === row.invoice_id) || {};
    ...
    if (maxPrice) matchesPrice = matchesPrice && Number(invoice.total) <= Number(maxPrice);
  ...
  const selectedPrintIds = selectedIds.map(id => {
    const printedInvoice = printed.find(p => p.id === id);
    const invoice = printInvoices.find(inv => inv && inv.id === printedInvoice?.invoice_id);
...
const handleDeletePrintedInvoice = async (id: number) => {
  if (!window.confirm('¿Seguro que quieres eliminar esta factura impresa?')) return;
  try {
  ...
  const columns = [
    {
      ...
      label: (
        <input
          type="checkbox"
          checked={selectedIds.length === filteredPrinted.length && filteredPrinted.length > 0}
    ...
    <div className="p-8 bg-gray-100 min-h-screen">
      {/* Contenedor oculto para impresión en lote */}
       <div style={{ position: 'absolute', left: '-9999px', top: 0 }}>
        {printed.map((printedItem, idx) => {
  const invoice = printInvoices.find(inv => inv && inv.id === printedItem.invoice_id);
  const company = printCompanies[printInvoices.findIndex(inv => inv && inv.id === printedItem.invoice_id)];
            ...
            >
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
                <div>
                  ...
                  <div style={{ ...baseTextStyle, margin: "2px 0" }}>Nº {printedItem.number}</div>
