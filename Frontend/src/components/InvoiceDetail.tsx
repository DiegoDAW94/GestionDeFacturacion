...
import { getInvoiceById, getCompanyById } from '../services/apiservices';
...
const InvoiceDetail: React.FC = () => {
  ...
  useEffect(() => {
    if (invoice?.company_id && token) {
      getCompanyById(invoice.company_id, token)
        .then(data => setCompany(data.company))
        .catch(() => setCompany(null));
    }
  }, [invoice?.company_id, token]);

  if (loading) return <div>Cargando factura...</div>;
  if (!invoice) return <div>No se encontró la factura.</div>;

  // Cambiado: Imprimir en vez de descargar
